...
import { getConfig, parseParams } from '../dist/parser'
...
test('name: name with resize and crop', t => {
    const params = parseParams(
        '/v1/ciwkuhq2s0dbf0131rcb3isiq/cj37jinmt008o0108zwhax711/600x200catt/0x0:20x20/Graphcool.jpg',
    )
...
test('resize: normal', t => {
    const config = getConfig({ resize: '500x300' })
    t.deepEqual(config, {
        resize: {
            width: 500,
            height: 300,
...
test('resize: smartcrop', t => {
    const config = getConfig({ resize: '500x300cc' })
    t.deepEqual(config, {
        resize: {
            width: 500,
            height: 300,
